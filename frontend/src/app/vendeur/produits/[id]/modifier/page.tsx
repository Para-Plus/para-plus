/**
 * Page de modification de produit (vendeur uniquement)
 */
...
import type { TypeProduit, Categorie, Produit } from '@/types/produit';
...
export default function ModifierProduitPage() {
  ...
  }, [user, authLoading, router, produitId]);

  const chargerDonnees = async () => {
    try {
      setLoadingData(true);
      const [categoriesData, produitsVendeur] = await Promise.all([
      ...
      const produitData = produitsVendeur.find(p => p.id === produitId);
      if (!produitData) {
      ...
      setFormData({
        ...
        prix: produitData.prix.toString(),
        prix_promo: produitData.prix_promo?.toString() || '',
        type_produit: produitData.type_produit,
        categorie_id: produitData.categorie_id,
        marque: produitData.marque || '',
        stock: produitData.stock.toString(),
        disponible_location: produitData.disponible_location,
        prix_location_jour: produitData.prix_location_jour?.toString() || '',
        images_urls: produitData.images.join(', '),
      });
    } catch (error: any) {
      setErreur(error.response?.data?.error || 'Erreur lors du chargement des données');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await produitsApi.modifier(produitId, produitData);

      // Rediriger vers le dashboard
      router.push('/vendeur/dashboard');
    } catch (error: any) {
      setErreur(error.response?.data?.error || 'Erreur lors de la modification du produit');
      setLoading(false);
    }
  };
  ...
  if (!user || user.role !== 'vendeur' || !produit) {
    return null;
  }
